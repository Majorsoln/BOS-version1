...
- **UI-06**'s bus policy detects the non-functional currency. The command is allowed because the FX bridge is recorded explicitly. The bridge is two events (per CN-5-103 G3 adapter convention + CN-5-001 N1 namespace fix): (a) `<fx-adapter>.fx.rate.recorded.v1 { rate_id, base_currency: KES, quote_currency: TZS, rate: 17.0, fx_event_date: 2026-05-12 }` from a Term 7 FX adapter (or future FX engine) supplies the authoritative rate; (b) `procurement.invoice.fx_applied.v1 { invoice_ref, rate_id, source_amount: 50000 KES, target_amount: 850000 TZS }` from Procurement records how that rate was applied to this specific invoice. The functional-currency invariant holds: every primary financial effect lands in TZS; the KES amount is bridged through explicit FX events with their own pack-version references (D-009).
